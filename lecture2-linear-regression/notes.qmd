...
Let's imagine we're designing a car and we would like to know what the fuel efficiency of the car we're designing will be in *miles per gallon* (MPG). We know some properties of our current design, such as the weight and horsepower, that we know that this should affect the efficiency. Ideally we would have access to a function that would give us the MPG rating if we provide these **features**.
...
plt.matshow(loss, cmap='coolwarm')
ax = plt.gca()
ax.set_xticks([])
ax.set_yticks([])

w, b = (12.5, 20)
other_lines = [(w * 1.2, b * 0.95), (w * 0.7, b * 0.9), (w * 1.8, b * 1.4)]
other_lines = [(w, b)] + other_lines
for param, color in zip(other_lines, [BLACK, BLUE, ORANGE, GREEN, PURPLE]):
            w, b = param
            ax.scatter(w, b, c=color, s=50)
plt.xlabel('$w_1$')
plt.ylabel('$b$')
...
Let's look at a more generic view of gradient descent. We have some function $f(\mathbf{\cdot})$ and we would like find the input $\mathbf{x}^*$ that minimizes the output of the function:

$$
\text{Find: } \mathbf{x}^* = \underset{\mathbf{x}}{\text{argmin}}\ f(\mathbf{x})
$$
...
class BasicFunction(PositiveVectorScene):
    def construct(self):
        fx = self.ax.plot(lambda x: (0.5 * x - 3) ** 2, color=RED)
        #eq.move_to(*self.ax.c2p([8, 7, 0]))
        labels = self.ax.get_axis_labels(x_label="x_1", y_label="y = f(\mathbf{x})")
        labels.set_color(GREY)
        labels.set_tex_template(self.template)
        p = Dot(color=BLACK).move_to(self.ax.c2p(6,0))
        label = MathTex('x^*', color=BLACK).next_to(p, UR)
        self.add(fx, labels, p, label)
        
```

We don't know how to find $\mathbf{x}^*$ directly, but if we have an initial guess $\mathbf{x}^{(0)}$, we can try to update our guess to improve it.

$$
\mathbf{x}^{(1)} \leftarrow \mathbf{x}^{(0)} + \mathbf{g}
$$
...
class BasicFunction(PositiveVectorScene):
    def construct(self):
        ...
        label = MathTex('x^{(0)}', color=BLACK).next_to(p, DR)
        self.add(fx, labels, p, label)
```

Here we are changing $\mathbf{x}^{(0)}$ by moving in the direction of $\mathbf{g}$. If we recall that the *gradient* of a function at point $\mathbf{x}$ corresponds to the *slope* of $f$ at $\mathbf{x}$, or equivalently the direction of maximum change. This gives us a natural choice for the update to our guess.

$$
\mathbf{x}^{(1)} \leftarrow \mathbf{x}^{(0)} - \nabla f(\mathbf{x}^{(0)})
$$

Note that because the gradient corresponds to the direction that maximally *increases* $f(\mathbf{x})$, we actually need to subtract the gradient in order to minimize our function. We can repeat this process many times, continuously updating our estimate.

$$
\text{For }i \text{ in 1,...,T}\text{ :} \\
\quad \mathbf{x}^{(i+1)} \leftarrow \mathbf{x}^{(i)} - \nabla f(\mathbf{x}^{(i)})
$$

## Gradient descent convergence

Recall that it's minimum value $\mathbf{x}^*$, a function $f$ *must* have a gradient of $\mathbf{0}$.
...
$$
\text{While } ||\nabla f(\mathbf{x}^{(i)})||_2 > \epsilon \text{ :} \\
\quad \mathbf{x}^{(i+1)} \leftarrow \mathbf{x}^{(i)} - \nabla f(\mathbf{x}^{(i)})
$$
...
Remember that the gradient is making a *linear approximation* to the function. A line has no minimum, so the gradient has no information about where along the approximation the true minimum will be.
...
$$
\mathbf{x}^{(i+1)} \leftarrow \mathbf{x}^{(i)} - \alpha \nabla f(\mathbf{x}^{(i)})
$$
...
The normal distribution shows up almost everywhere in probability and statistics. Most notably, the *central limit theorem* tells us that the sum of many independent and identically distributed random outcomes tends towards a normal distribution.
