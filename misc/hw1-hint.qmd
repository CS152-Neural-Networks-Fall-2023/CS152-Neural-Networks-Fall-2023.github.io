...
Therefore, if we saw such a summation in an expression, we could temporarily replace it with $c_i$, knowing that we defined $\mathbf{c}$ as $\mathbf{c}=\mathbf{A}\mathbf{x}$. Try doing this as the first step in the homework.

Also recall as well that a *dot product* between two vectors $\mathbf{x}$ and $\mathbf{y}$, can be written explicitly from its definition as:
