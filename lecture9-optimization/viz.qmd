...
def PlotlyFigure(width=800, height=None, hide_toolbar=True):
  if height is None:
    height = 0.75 * width
...
def PlotlyInput(width=800, height=None, hide_toolbar=True):
...
wrange = tf.linspace(-3, 3, 50)
brange = tf.linspace(-3, 3, 50)
...
losscontour = dict(x=wrange, y=brange, z=lossgrid, type='contour', ncontours=25, )

batchlossgrid = loss(paramgrid, xbatch, ybatch).reshape(ww.shape)
batchlosscontour = dict(x=wrange, y=brange, z=batchlossgrid, type='contour', ncontours=25,)


cweights = ${weights}
startpoint = dict(x=[cweights[0]], y=[cweights[1]], mode='markers', marker=dict(color='firebrick', size=10, line= {'color': 'black', 'width': 3}))
...
for i, (nxbatch, nybatch) in zip(range(steps), batches):
  ...
  if beta > 0:
    bgrad = bgrad / tf.sqrt(magnitude + 1e-8)
  batchgrad = (bgrad * (1. - momentum)) + momentum * batchgrad
...
gradplot = dict(x=fullweights[:, 0], y=fullweights[:, 1], line=dict(color='black'))
batchgradplot = dict(x=batchweights[:, 0], y=batchweights[:, 1], line=dict(color='orange'))
